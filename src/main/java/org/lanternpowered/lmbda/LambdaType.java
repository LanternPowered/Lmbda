...
public abstract class LambdaType<@NonNull T> {
   ...
   *
   * <p>A functional abstract class is either a interface or a "abstract" class that has a no-arg
   * constructor. In both cases is only one abstract method allowed.</p>
   *
  ...
  /**
   * Constructs a new {@link LambdaType} from the given function interface or abstract class type.
   *
   * <p>A functional abstract class is either a interface or a "abstract" class that has a no-arg
   * constructor. In both cases is only one abstract method allowed.</p>
   *
   * <p>The generic signature of the given {@link Type} will be retained by a resulting function
   * .</p>
   *
   * @param functionType The function type
  ...
  /**
   * The lookup the generated lambda implementation will be defined in. This is required when the
   * target interface doesn't provide public access that it can be implemented outside its package.
   */
  final MethodHandles.@Nullable Lookup defineLookup;
   ...
   * Constructs a new {@link LambdaType}.
   *
   * <p>The generic signature from the extended class will be used to determine the functional
   * interface to implement. If it's not resolved, a {@link IllegalStateException} can be
   * expected.</p>
   */
  public LambdaType() {
  ...
  /**
   * Constructs a new {@link LambdaType} where that will define the implementation class using
   * the provided lookup.
   *
   * <p>When the functional interface/abstract class isn't public, or the abstract method isn't,
   * then should the lookup be defined explicitly. Its implementation class needs the proper
   * access in order to be implemented successfully. This can be in the same package or a
   * different one depending on what is desired.</p>
   *
   * @param defineLookup The define lookup
  ...
  /**
   * Gets the {@link Method} that will be implemented when generating a function for this
   * {@link LambdaType}.
   *
   * @return The method
   */
